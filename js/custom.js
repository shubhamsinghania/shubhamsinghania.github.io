...
/* ================= HANDLE UPLOAD (FIXED) ================= */
// Inside your uploadBtn.onclick loop...

try {
    const uploadInfo = await requestPresignedUploadUrl(knowledgeSourceId, sync.id, safeFileName);
    
    // 1. Get the target S3 URL
    const s3Url = uploadInfo.url;
    
    // 2. Prepare the headers exactly as Genesys/S3 expects them
    // This includes the x-amz-meta tags provided in the response
    const uploadHeaders = {
        ...uploadInfo.headers,
        'Content-Type': file.type || 'application/octet-stream'
    };

    // 3. Execute the PUT request directly to S3
    const s3Response = await fetch(s3Url, {
        method: 'PUT',
        headers: uploadHeaders,
        body: file // Send the raw File object/blob
    });

    if (!s3Response.ok) {
        const errorBody = await s3Response.text();
        addUploadResult(file.name, 'error', `S3 rejected upload: ${s3Response.status}`);
        console.error('S3 Error Details:', errorBody);
        continue;
    }
    ...
    addUploadResult(file.name, 'error', `Process failed: ${err.message}`);
}
        }
